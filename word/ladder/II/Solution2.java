...
public class Solution2 {
	...
	public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
        ...
        int count = 2;
        Hashtable<String, HashSet<String>> backwardRoute = new Hashtable<String, HashSet<String>>();
        while(!moveOneStepForward(camps, dict, end, backwardRoute)) {
        ...
        //log back and get a complete solution
        ArrayList<ArrayList<String>> solutions = new ArrayList<ArrayList<String>>();
        ArrayList<String> route = new ArrayList<String>();
        route.add(end);
        logBack(solutions, backwardRoute, end, start, route);
        return solutions;
    }
	
	//return true if target is found
	private boolean moveOneStepForward(
			Queue<String> camps, 
			HashSet<String> availableNodes, 
			String target,
			Hashtable<String, HashSet<String>> backwardRoute) {
	...
	private void logBack(
			ArrayList<ArrayList<String>> solution,//stores final solutions 
			Hashtable<String, HashSet<String>> backlog,//stores key -> list of next step pairs
			String currentKey,
			String target,//original start
			ArrayList<String> route//stores one potential path through this word
			) {
		
		if(solution == null || solution == null || currentKey == null || route == null) {
			throw new RuntimeException("Invalid inputs for logBack() funtion!");
		}
		
		if(currentKey.equalsIgnoreCase(target)) {//find a path through which end and start is connected
			route.add(target);
			ArrayList<String> _path = (ArrayList<String>)route.clone();
			Collections.reverse(_path);
			solution.add(_path);
			return;
		}
		
		HashSet<String> backwardWords = backlog.get(currentKey);
	
		for(String _next : backwardWords) {
			route.add(_next);
			logBack(solution, backlog, _next, target, route);
			route.remove(_next);
		}
		return;
	}
